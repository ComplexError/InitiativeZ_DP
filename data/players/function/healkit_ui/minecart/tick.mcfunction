...
# save the current tick's Items over previous, if the are able to then something has changed
execute on passengers store result score #bool ui run data modify entity @s data.previous set from storage ui current

execute if score #bool ui matches 1 run function players:healkit_ui/minecart/on_change