...
execute if score $filter find_look.temp matches 1.. as @e[tag=find_looking.candidate,tag=find_looking.result] run function master:look_find/internal/filter/check_filter
...
execute store success score $success find_look.temp if predicate master:can_see_filter
...
execute if entity @e[tag=find_looking.candidate,tag=find_looking.result,limit=1] if score $filter find_look.temp <= $max find_look.temp run function master:look_find/internal/filter/iteration
